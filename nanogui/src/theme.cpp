...
#include <filesystem>
// Note: the filesystem is not standardized yet, and this namespace may work on MSVC only
...
Theme::Theme(NVGcontext *ctx) {
    ...
    if (stdfs::exists(stdfs::path("NotoSansTC-Regular.otf")) &&
        stdfs::exists(stdfs::path("NotoSansTC-Bold.otf")))
    {
        mFontNormal = nvgCreateFont(ctx, "sans", "NotoSansTC-Regular.otf");
        mFontBold = nvgCreateFont(ctx, "sans-bold", "NotoSansTC-Bold.otf");
    }
    else
    {
